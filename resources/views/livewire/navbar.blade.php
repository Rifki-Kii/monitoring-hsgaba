<nav class="w-full bg-white border-b border-gray-200 px-6 py-3 flex items-center justify-between sticky top-0 z-30 shadow-sm" x-data="{ userOpen: false, notifOpen: false }">
    ...
    <div class="flex items-center space-x-4">
        ...
        <div class="relative">
            <button @click="userOpen = !userOpen; notifOpen = false" class="flex items-center space-x-3 p-1 rounded-lg hover:bg-gray-100 transition-colors group">
                {{-- Avatar Dinamis --}}
                <div class="w-8 h-8 rounded-full bg-gradient-to-r from-blue-500 to-indigo-500 flex items-center justify-center text-white text-sm font-medium uppercase">
                ...
                <div class="hidden md:block text-left">
                    ...
                    {{-- FIX: Role diambil dari database, bukan tulisan statis --}}
            ...
            <div x-show="userOpen" @click.away="userOpen = false" class="hidden absolute right-0 mt-2 w-56 bg-white rounded-xl shadow-lg border border-gray-200 z-40" :class="{'hidden': !userOpen}">
                <div class="p-4 border-b border-gray-100">
                    ...
                    <p class="text-sm text-gray-500 mt-1 truncate">{{ auth()->user()->email }}</p>
                </div>

                <div class="py-2">
                    {{-- MENU 1: BROADCAST WA (Hanya Admin & Wali Kelas) --}}
    ...
    {{-- MODAL WA (Hanya di-render jika User memiliki akses, untuk keamanan & performa) --}}
    @if(in_array(auth()->user()->role, ['admin', 'wali_kelas']))
        @if($showWaModal)
    ...
    <script>
        document.addEventListener('livewire:initialized', () => {
            @this.on('notify-success', (event) => { alert(event.message); });
            @this.on('notify-error', (event) => { alert(event.message); });
        });
    </script>
</nav>